...
def draw(csv_file, ax):
    with open('filetest.csv', 'r') as file:
        ...
        for row in reader:
            ...
            ax.text(x1, y1, f'(P{order_index})', ha='right', va='bottom', fontsize=10)
            ax.text(x2, y2, f'(D{order_index})', ha='left', va='bottom', fontsize=10)
            order_index += 1
    x = []
    y = []
...
csv_file1 = 'outputGen.csv'
csv_file2 = 'outputOp.csv'
